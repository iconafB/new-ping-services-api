...
async def load_credits(credits_amount:CreateCredits,client:CurrentClientSchema=Depends(get_current_active_user),session:AsyncSession=Depends(get_async_session)):
    ...
    """
    return await credits_object.load_client_credits(credits_amount=credits_amount.credits_amount,user_id=client.client_id,session=session)
...
async def get_user_credits_history(page:int=Query(1,ge=1,description="page number"),page_size:int=Query(10,le=100,description="page size"),client:CurrentClientSchema=Depends(get_current_active_user),session:AsyncSession=Depends(get_async_session)):
    return await credits_object.get_all_credits_history_for_a_user(page=page,page_size=page_size,user_id=client.client_id,session=session)
...
async def delete_credits_history_for_current_user(client:CurrentClientSchema=Depends(get_current_active_user),session:AsyncSession=Depends(get_async_session)):
    return await credits_object.delete_credits_history_db(user_id=client.client_id,session=session)
...
async def withdrawals(client:CurrentClientSchema=Depends(get_current_active_user),session:AsyncSession=Depends(get_async_session),page:int=Query(1,ge=1,description="Page Number"),page_size:int=Query(10,le=100,description="Number of items per unit page")):
    """
...
async def download_credits_statements_pdf(client:CurrentClientSchema=Depends(get_current_active_user),start_date:date | None=Query(default=None,description="Filter statement from this date (inclusive). Format: YYYY-MM-DD",openapi_examples={ "month_start": {
