...
async def get_all_clients(session:AsyncSession=Depends(get_async_session),page:int=Query(1,description="Page Number",ge=1),page_size:int=Query(10,description="Page size",le=100),user_id=Depends(get_current_active_user_id)):
...
async def get_all_active_clients(session:AsyncSession=Depends(get_async_session),page:int=Query(1,ge=1,description="Page number"),page_size:int=Query(10,le=100,description="Provide page size number"),user_id=Depends(get_current_active_user_id)):
...
async def get_single_client(session:AsyncSession=Depends(get_async_session),client_id:int=Path(description="Provide client id"),user_id=Depends(get_current_active_user_id)):
...
async def deleted_client(session:AsyncSession=Depends(get_async_session),client_id:int=Path(description="client id"),user_id=Depends(get_current_active_user_id)):
    """
...
async def update_client_information(client:UpdateClientSchema,session:AsyncSession=Depends(get_async_session),client_id:int=Path(description="Provide client id"),user_id=Depends(get_current_active_user_id)):
    ...
    return await clients_object.update_single_client(client=client,client_id=client_id,session=session)
